...
///////////////////////////////////////////////////////////////////
    void printMessage(const std::string& message) {
        mvwprintw(input_win, 1, 1, "%s", message.c_str());  // Print the message inside the box
        wrefresh(input_win);  // Refresh to show the changes
    }
    ...
    while ((ch = wgetch(input_win)) != '\n') {   // '\n' represents the Enter key
        if (ch == KEY_BACKSPACE || ch == 127) {  // Handle backspace
            if (!input.empty()) {
                ...
                mvwprintw(input_win, 1, 7 + prompt.length(), "%s ", input.c_str());
            }
        } else if (ch >= 32 && ch <= 126 && input.size() < box_width - 8) {
